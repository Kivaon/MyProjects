...
def merge_nikud_with_letters_clean(words: List[Dict]) -> List[Dict]:
    """Объединяет никуд с буквами и убирает пустые символы"""
    if not words:
        return words
    ...
    while i < len(sorted_words):
        ...
        if is_nikud(current_word['text']):
            ...
            if best_match is not None and best_distance <= 15.0:  # Увеличиваем допуск по X
                ...
                # Пропускаем никуд (не добавляем в merged_words)
                i += 1
                continue
        
        # Если это обычное слово или никуд не удалось присоединить
        # Проверяем, не является ли слово пустым после объединения
...
@dataclass
class ProcessedWord:
    """Обработанное слово с чистым объединением никуда"""
    text: str
    original_text: str
    x0: float
    y0: float
...
@dataclass
class ProcessedLine:
    """Обработанная строка с чистым объединением никуда"""
...
class NikudCleanAnalyzer:
    """Анализатор с чистым объединением никуда"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_nikud_clean(self, pdf_path: str) -> bool:
        """Основной метод анализа с чистым объединением никуда"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_nikud_clean(page)
    ...
    def _analyze_page_with_nikud_clean(self, page):
        """Анализ страницы с чистым объединением никуда"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И ЧИСТОЕ ОБЪЕДИНЕНИЕ НИКУДА")
        print("="*60)
        
        # Извлекаем слова
        raw_words = page.extract_words()
        
        # Чистое объединение никуда с буквами
        merged_words = merge_nikud_with_letters_clean(raw_words)
        
        print(f"  📊 До объединения: {len(raw_words)} слов")
        print(f"  🔗 После объединения: {len(merged_words)} слов")
        print(f"  📝 Убрано пустых символов: {len(raw_words) - len(merged_words)}")
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С ЧИСТЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с чистым объединением никуда
        processed_words = self._process_words_with_nikud_clean(merged_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ЧИСТЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с чистым объединением никуда
        y_groups = self._form_lines_with_nikud_clean(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ЧИСТЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 3: Статистика с чистым объединением никуда
        self._analyze_with_nikud_clean_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_nikud_clean_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_nikud_clean(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с чистым объединением никуда"""
        print(f"  🔤 Обработка {len(raw_words)} слов с чистым объединением никуда:")
    ...
    def _form_lines_with_nikud_clean(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с чистым объединением никуда"""
        print(f"  📋 Формирование строк с чистым объединением никуда ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_nikud_clean(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_nikud_clean(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с чистым объединением никуда"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_nikud_clean(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_nikud_clean(line)
    ...
    def _split_words_into_lines_nikud_clean(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с чистым объединением никуда"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_nikud_clean(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с чистым объединением никуда"""
        if not words:
            return ""
    ...
    def _analyze_with_nikud_clean_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с чистым объединением никуда"""
        print(f"  📊 Анализ {len(y_groups)} групп с чистым объединением никуда:")
    ...
    def _save_nikud_clean_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с чистым объединением никуда"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с чистым объединением никуда
        groups_file = os.path.join(debug_dir, f"{timestamp}_nikud_clean_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ЧИСТЫМ ОБЪЕДИНЕНИЕМ НИКУДА\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с чистым объединением никуда сохранен:")
        print(f"     📄 Группы с чистым объединением никуда: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_nikud_clean.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = NikudCleanAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_nikud_clean(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с чистым объединением никуда завершен успешно")
    else:
