...
def analyze_group7_gap(pdf_path: str):
    """Анализирует разрывы в группе 7 между словами שמות и כאשר"""
    try:
        with pdfplumber.open(pdf_path) as pdf:
            if pdf.pages:
                ...
                print("ГРУППА 7 - АНАЛИЗ РАЗРЫВОВ")
                print("="*60)
                ...
                for i in range(len(group7_words) - 1):
                    ...
                    print(f"  Текущее слово: X={current_word['x0']:.1f}-{current_word['x1']:.1f}")
                    print(f"  Следующее слово: X={next_word['x0']:.1f}-{next_word['x1']:.1f}")
                ...
                if shemot_word and kaasher_word:
                    ...
                    print(f"שמות: X={shemot_word['x0']:.1f}-{shemot_word['x1']:.1f}")
                    print(f"כאשר: X={kaasher_word['x0']:.1f}-{kaasher_word['x1']:.1f}")
                    print(f"Разрыв: {gap:.1f} пикселей")
...
def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_debug_group7_gap.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    success = analyze_group7_gap(pdf_path)
