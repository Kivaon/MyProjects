...
# --- ПАСПОРТ ---
VERSION = "v3.11.hebrew-punctuation-final"
DATE    = "2026-05-01"
...
# Правильные ивритские знаки препинания (Unicode)
HEBREW_PUNCTUATION = {
    # Основные знаки препинания
    '.': '\u05F3',      # Ивритская точка (Mafsiq)
    ',': '\u05F3',      # Ивритская запятая (Mafsiq)
    ':': '\u05F4',      # Ивритское двоеточие (Gershayim)
    ';': '\u05F3',      # Ивритская точка с запятой (Mafsiq)
    '!': '\u05BF',      # Восклицательный знак (Rafe)
    '?': '\u05BF',      # Вопросительный знак (Rafe)
    
    # Кавычки
    '"': '\u05F4',      # Ивритские кавычки (Gershayim)
    "'": '\u05F3',      # Ивритские одинарные кавычки (Mafsiq)
    
    # Скобки (используются стандартные Unicode)
...
def normalize_hebrew_punctuation(text: str) -> str:
    """Нормализует английские знаки препинания в ивритские"""
    result = []
    for char in text:
        if char in HEBREW_PUNCTUATION:
            result.append(HEBREW_PUNCTUATION[char])
        else:
            result.append(char)
    return ''.join(result)
...
@dataclass
class ProcessedWord:
    """Обработанное слово с правильными ивритскими знаками препинания"""
    text: str
    original_text: str
    x0: float
    y0: float
...
@dataclass
class ProcessedLine:
    """Обработанная строка с правильными ивритскими знаками препинания"""
...
class HebrewPunctuationFinalAnalyzer:
    """Анализатор с финальными исправленными ивритскими знаками препинания"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_hebrew_punctuation_final(self, pdf_path: str) -> bool:
        """Основной метод анализа с финальными ивритскими знаками препинания"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_hebrew_punctuation_final(page)
    ...
    def _analyze_page_with_hebrew_punctuation_final(self, page):
        """Анализ страницы с финальными ивритскими знаками препинания"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И ФИНАЛЬНЫХ ИВРИТСКИХ ЗНАКОВ")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С ФИНАЛЬНЫМИ ИВРИТСКИМИ ЗНАКАМИ")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с финальными ивритскими знаками
        processed_words = self._process_words_with_hebrew_punctuation_final(words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ФИНАЛЬНЫМИ ИВРИТСКИМИ ЗНАКАМИ")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с финальными ивритскими знаками
        y_groups = self._form_lines_with_hebrew_punctuation_final(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ФИНАЛЬНЫМИ ИВРИТСКИМИ ЗНАКАМИ")
        print("="*60)
        
        # ЭТАП 3: Статистика с финальными ивритскими знаками
        self._analyze_with_hebrew_punctuation_final_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_hebrew_punctuation_final_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_hebrew_punctuation_final(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с финальными ивритскими знаками препинания"""
        print(f"  🔤 Обработка {len(raw_words)} слов с финальными ивритскими знаками препинания:")
        ...
        for i, word in enumerate(raw_words):
            ...
            # Нормализуем знаки препинания для иврита
            if self.language_info['language'] == 'hebrew':
                processed_text = normalize_hebrew_punctuation(original_text)
            else:
            ...
            if i < 15:
                ...
                if original_text != processed_text:
                    ...
                    change_mark = ""
                print(f"    {i+1:2d}. {original_text:12s}{change_mark:15s} {mark} ({word['x0']:6.1f},{word['top']:6.1f})")
            elif i == 15:
    ...
    def _form_lines_with_hebrew_punctuation_final(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с финальными ивритскими знаками препинания"""
        print(f"  📋 Формирование строк с финальными ивритскими знаками препинания ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_hebrew_punctuation_final(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_hebrew_punctuation_final(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с финальными ивритскими знаками препинания"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_hebrew_punctuation_final(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_hebrew_punctuation_final(line)
    ...
    def _split_words_into_lines_hebrew_punctuation_final(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с финальными ивритскими знаками препинания"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_hebrew_punctuation_final(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с финальными ивритскими знаками препинания"""
        if not words:
            return ""
        ...
        if direction == 'rtl':
            ...
            # Собираем текст без пробелов между словами и знаками препинания
            text_parts = []
    ...
    def _analyze_with_hebrew_punctuation_final_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с финальными ивритскими знаками препинания"""
        print(f"  📊 Анализ {len(y_groups)} групп с финальными ивритскими знаками препинания:")
    ...
    def _save_hebrew_punctuation_final_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с финальными ивритскими знаками препинания"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с финальными ивритскими знаками препинания
        groups_file = os.path.join(debug_dir, f"{timestamp}_hebrew_punctuation_final_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ФИНАЛЬНЫМИ ИВРИТСКИМИ ЗНАКАМИ ПРЕПИНАНИЯ\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с финальными ивритскими знаками препинания сохранен:")
        print(f"     📄 Группы с финальными ивритскими знаками препинания: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_hebrew_punctuation_final.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = HebrewPunctuationFinalAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_hebrew_punctuation_final(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с финальными ивритскими знаками препинания завершен успешно")
    else:
