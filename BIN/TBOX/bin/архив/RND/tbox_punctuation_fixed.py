...
# --- ПАСПОРТ ---
VERSION = "v3.7.punctuation-fixed"
DATE    = "2026-05-01"
...
@dataclass
class ProcessedWord:
    """Обработанное слово с учетом знаков препинания"""
    text: str
    original_text: str
    x0: float
    y0: float
    ...
    is_punctuation: bool
    should_attach: bool  # Нужно ли присоединять к предыдущему слову

@dataclass
class ProcessedLine:
    """Обработанная строка с правильными знаками препинания"""
...
class PunctuationFixedAnalyzer:
    """Анализатор с исправленной обработкой знаков препинания"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_punctuation_fix(self, pdf_path: str) -> bool:
        """Основной метод анализа с исправленными знаками препинания"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_punctuation_fix(page)
    ...
    def _analyze_page_with_punctuation_fix(self, page):
        """Анализ страницы с исправленными знаками препинания"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И АНАЛИЗ ЗНАКОВ ПРЕПИНАНИЯ")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ИСПРАВЛЕННАЯ ОБРАБОТКА ЗНАКОВ ПРЕПИНАНИЯ")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с исправленными знаками препинания
        processed_words = self._process_words_with_punctuation_fix(words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ИСПРАВЛЕННЫМИ ЗНАКАМИ")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с исправленными знаками
        y_groups = self._form_lines_with_punctuation_fix(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ИСПРАВЛЕННЫМИ ЗНАКАМИ")
        print("="*60)
        
        # ЭТАП 3: Статистика с исправленными знаками
        self._analyze_with_punctuation_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_punctuation_fixed_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_punctuation_fix(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с исправленными знаками препинания"""
        print(f"  🔤 Обработка {len(raw_words)} слов с исправленными знаками препинания:")
        ...
        for i, word in enumerate(raw_words):
            ...
            # Определяем, нужно ли присоединять к предыдущему слову
            should_attach = False
            if word_is_punctuation and direction == 'rtl':
                # Для RTL: знаки препинания обычно присоединяются к предыдущему слову
                should_attach = True
            ...
            if i < 15:
                ...
                attach_mark = "🔗" if should_attach else "  "
                print(f"    {i+1:2d}. {original_text:12s} → {processed_text:12s} {mark}{attach_mark} ({word['x0']:6.1f},{word['top']:6.1f})")
            elif i == 15:
        ...
        print(f"  🔗 Для присоединения: {sum(1 for w in processed_words if w.should_attach)}")
        
        return processed_words
    
    def _form_lines_with_punctuation_fix(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с исправленными знаками препинания"""
        print(f"  📋 Формирование строк с исправленными знаками препинания:")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_punctuation_fix(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_punctuation_fix(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с исправленными знаками препинания"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_punctuation_fix(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                # Формируем текст с правильными знаками препинания
                text = self._assemble_text_with_punctuation_fix(line)
    ...
    def _split_words_into_lines_punctuation_fix(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с исправленными знаками препинания"""
        if len(words) <= 1:
            return [words]
        ...
        # Находим разрывы между словами с учетом знаков препинания
        lines = []
        ...
        for i in range(len(sorted_words) - 1):
            ...
            if next_word.is_punctuation or current_word.is_punctuation:
                effective_threshold = gap_threshold * 0.5  # Знаки препинания могут быть ближе
    ...
    def _assemble_text_with_punctuation_fix(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с правильными знаками препинания"""
        if not words:
            return ""
        
        direction = self.language_info['direction']
        
        # Разделяем слова на обычные и знаки препинания
        regular_words = []
        punctuation_words = []
        ...
        if direction == 'rtl':
            # Для RTL: знаки препинания присоединяются к предыдущему слову
            text_parts = []
            for word in words:
                if word.is_punctuation:
                    # Присоединяем знак препинания к предыдущему слову
                    if text_parts:
            ...
            # Для LTR: стандартное соединение через пробел
            return ' '.join(w.text for w in words)
    
    def _analyze_with_punctuation_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с исправленными знаками препинания"""
        print(f"  📊 Анализ {len(y_groups)} групп с исправленными знаками препинания:")
    ...
    def _save_punctuation_fixed_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с исправленными знаками препинания"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с исправленными знаками препинания
        groups_file = os.path.join(debug_dir, f"{timestamp}_punctuation_fixed_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ИСПРАВЛЕННЫМИ ЗНАКАМИ ПРЕПИНАНИЯ\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с исправленными знаками сохранен:")
        print(f"     📄 Группы с исправленными знаками: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_punctuation_fixed.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = PunctuationFixedAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_punctuation_fix(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с исправленными знаками препинания завершен успешно")
    else:
