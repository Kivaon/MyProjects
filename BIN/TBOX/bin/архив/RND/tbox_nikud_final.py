...
def merge_nikud_with_letters_final(words: List[Dict]) -> List[Dict]:
    """Финальное объединение никуда с буквами - максимально агрессивное"""
    if not words:
        return words
    ...
    # Создаем карту слов по Y координатам
    y_groups = defaultdict(list)
    for word in sorted_words:
        y_key = round(word['top'] / 2) * 2  # Группируем по 2 пикселям
        y_groups[y_key].append(word)
    
    merged_words = []
    
    for y_key, y_words in y_groups.items():
        # Обрабатываем каждую Y группу отдельно
        group_merged = []
        
        for word in y_words:
            if is_nikud(word['text']):
                # Ищем ближайшее слово в той же Y группе
                best_match = None
                min_distance = float('inf')
                
                for candidate in group_merged:
                    # Проверяем Y близость
                    if abs(word['top'] - candidate['top']) <= 8.0:  # Увеличиваем Y допуск
                        # Вычисляем X расстояние
                        if word['x0'] >= candidate['x0'] and word['x0'] <= candidate['x1']:
                            # Никуд внутри слова
                            distance = 0
                        elif word['x0'] < candidate['x0']:
                            # Никуд слева
                            distance = candidate['x0'] - word['x0']
                        else:
                            # Никуд справа
                            distance = word['x0'] - candidate['x1']
                        
                        if distance < min_distance and distance <= 20.0:  # Увеличиваем X допуск
                ...
                if best_match:
                    ...
                    # Расширяем X диапазон
                    best_match['x1'] = max(best_match['x1'], word['x1'])
                else:
                    # Не нашли слово - добавляем как отдельное
                    group_merged.append(word.copy())
            else:
                # Обычное слово
                group_merged.append(word.copy())
        
        # Добавляем объединенные слова из группы
        merged_words.extend(group_merged)
    
    # Финальная очистка - убираем пустые слова
    final_words = []
...
@dataclass
class ProcessedWord:
    """Обработанное слово с финальным объединением никуда"""
    text: str
    original_text: str
    x0: float
    y0: float
...
@dataclass
class ProcessedLine:
    """Обработанная строка с финальным объединением никуда"""
...
class NikudFinalAnalyzer:
    """Анализатор с финальным объединением никуда"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_nikud_final(self, pdf_path: str) -> bool:
        """Основной метод анализа с финальным объединением никуда"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_nikud_final(page)
    ...
    def _analyze_page_with_nikud_final(self, page):
        """Анализ страницы с финальным объединением никуда"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И ФИНАЛЬНОЕ ОБЪЕДИНЕНИЕ НИКУДА")
        print("="*60)
        
        # Извлекаем слова
        raw_words = page.extract_words()
        
        # Финальное объединение никуда с буквами
        merged_words = merge_nikud_with_letters_final(raw_words)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С ФИНАЛЬНЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с финальным объединением никуда
        processed_words = self._process_words_with_nikud_final(merged_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ФИНАЛЬНЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с финальным объединением никуда
        y_groups = self._form_lines_with_nikud_final(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ФИНАЛЬНЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 3: Статистика с финальным объединением никуда
        self._analyze_with_nikud_final_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_nikud_final_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_nikud_final(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с финальным объединением никуда"""
        print(f"  🔤 Обработка {len(raw_words)} слов с финальным объединением никуда:")
    ...
    def _form_lines_with_nikud_final(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с финальным объединением никуда"""
        print(f"  📋 Формирование строк с финальным объединением никуда ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_nikud_final(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_nikud_final(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с финальным объединением никуда"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_nikud_final(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_nikud_final(line)
    ...
    def _split_words_into_lines_nikud_final(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с финальным объединением никуда"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_nikud_final(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с финальным объединением никуда"""
        if not words:
            return ""
    ...
    def _analyze_with_nikud_final_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с финальным объединением никуда"""
        print(f"  📊 Анализ {len(y_groups)} групп с финальным объединением никуда:")
    ...
    def _save_nikud_final_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с финальным объединением никуда"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с финальным объединением никуда
        groups_file = os.path.join(debug_dir, f"{timestamp}_nikud_final_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ФИНАЛЬНЫМ ОБЪЕДИНЕНИЕМ НИКУДА\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с финальным объединением никуда сохранен:")
        print(f"     📄 Группы с финальным объединением никуда: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_nikud_final.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = NikudFinalAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_nikud_final(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с финальным объединением никуда завершен успешно")
    else:
