...
# --- ПАСПОРТ ---
VERSION = "v3.9.hebrew-punctuation"
DATE    = "2026-05-01"
...
# Ивритские знаки препинания (Unicode)
HEBREW_PUNCTUATION = {
    '.': '\u05E4\u05BC\u05E9\u05B5\u05D9\u05E8',  # Ивритская точка (Geresh)
    ',': '\u05F3',  # Ивритская запятая
    ':': '\u05F4',  # Ивритское двоеточие
    ';': '\u05F3',  # Ивритская точка с запятой
    '!': '\u05BF',  # Ивритский восклицательный знак
    '?': '\u05BF',  # Ивритский вопросительный знак
    '"': '\u05F4',  # Ивритские кавычки
    "'": '\u05F3',  # Ивритские одинарные кавычки
}
...
@dataclass
class ProcessedWord:
    """Обработанное слово с ивритскими знаками препинания"""
    text: str
    original_text: str
    x0: float
    y0: float
...
@dataclass
class ProcessedLine:
    """Обработанная строка с ивритскими знаками препинания"""
...
class HebrewPunctuationAnalyzer:
    """Анализатор с ивритскими знаками препинания"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_hebrew_punctuation(self, pdf_path: str) -> bool:
        """Основной метод анализа с ивритскими знаками препинания"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_hebrew_punctuation(page)
    ...
    def _analyze_page_with_hebrew_punctuation(self, page):
        """Анализ страницы с ивритскими знаками препинания"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И ИВРИТСКИХ ЗНАКОВ ПРЕПИНАНИЯ")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С ИВРИТСКИМИ ЗНАКАМИ ПРЕПИНАНИЯ")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с ивритскими знаками препинания
        processed_words = self._process_words_with_hebrew_punctuation(words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ИВРИТСКИМИ ЗНАКАМИ")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с ивритскими знаками
        y_groups = self._form_lines_with_hebrew_punctuation(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ИВРИТСКИМИ ЗНАКАМИ")
        print("="*60)
        
        # ЭТАП 3: Статистика с ивритскими знаками
        self._analyze_with_hebrew_punctuation_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_hebrew_punctuation_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_hebrew_punctuation(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с ивритскими знаками препинания"""
        print(f"  🔤 Обработка {len(raw_words)} слов с ивритскими знаками препинания:")
    ...
    def _form_lines_with_hebrew_punctuation(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с ивритскими знаками препинания"""
        print(f"  📋 Формирование строк с ивритскими знаками препинания ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_hebrew_punctuation(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_hebrew_punctuation(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с ивритскими знаками препинания"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_hebrew_punctuation(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_hebrew_punctuation(line)
    ...
    def _split_words_into_lines_hebrew_punctuation(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с ивритскими знаками препинания"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_hebrew_punctuation(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с ивритскими знаками препинания"""
        if not words:
            return ""
    ...
    def _analyze_with_hebrew_punctuation_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с ивритскими знаками препинания"""
        print(f"  📊 Анализ {len(y_groups)} групп с ивритскими знаками препинания:")
    ...
    def _save_hebrew_punctuation_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с ивритскими знаками препинания"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с ивритскими знаками препинания
        groups_file = os.path.join(debug_dir, f"{timestamp}_hebrew_punctuation_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ИВРИТСКИМИ ЗНАКАМИ ПРЕПИНАНИЯ\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с ивритскими знаками препинания сохранен:")
        print(f"     📄 Группы с ивритскими знаками препинания: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_hebrew_punctuation.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = HebrewPunctuationAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_hebrew_punctuation(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с ивритскими знаками препинания завершен успешно")
    else:
