...
def merge_nikud_with_letters_enhanced(words: List[Dict]) -> List[Dict]:
    """Улучшенное объединение никуда с буквами на основе близости X координат"""
    if not words:
        return words
    ...
    while i < len(sorted_words):
        ...
        # Если это никуд, пытаемся присоединить к ближайшему предыдущему слову
        if is_nikud(current_word['text']):
            # Ищем ближайшее предыдущее слово с близкими Y координатами
            best_match = None
            best_distance = float('inf')
            
            for j in range(len(merged_words) - 1, -1, -1):
                prev_word = merged_words[j]
                y_tolerance = 5.0  # Увеличиваем допуск по Y
                
                if abs(current_word['top'] - prev_word['top']) <= y_tolerance:
                    # Вычисляем расстояние по X
                    if current_word['x0'] >= prev_word['x0'] and current_word['x0'] <= prev_word['x1']:
                        # Никуд находится внутри X диапазона слова
                        distance = 0
                    elif current_word['x0'] < prev_word['x0']:
                        # Никуд слева от слова
                        distance = prev_word['x0'] - current_word['x0']
                    else:
                        # Никуд справа от слова
                        distance = current_word['x0'] - prev_word['x1']
                    
                    if distance < best_distance:
                        best_distance = distance
                        best_match = j
            
            # Если нашли подходящее слово и расстояние достаточно мало
            if best_match is not None and best_distance <= 10.0:  # Увеличиваем допуск по X
                # Присоединяем никуд к найденному слову
                merged_words[best_match]['text'] += current_word['text']
                # Обновляем X координаты
                merged_words[best_match]['x1'] = max(
                    merged_words[best_match]['x1'], 
                    current_word['x1']
                )
                i += 1
                continue
        
        # Если это обычное слово или никуд не удалось присоединить
        merged_words.append(current_word.copy())
        i += 1
...
@dataclass
class ProcessedWord:
    """Обработанное слово с улучшенным объединением никуда"""
    text: str
    original_text: str
    x0: float
    y0: float
...
@dataclass
class ProcessedLine:
    """Обработанная строка с улучшенным объединением никуда"""
...
class NikudEnhancedAnalyzer:
    """Анализатор с улучшенным объединением никуда"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_nikud_enhanced(self, pdf_path: str) -> bool:
        """Основной метод анализа с улучшенным объединением никуда"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_nikud_enhanced(page)
    ...
    def _analyze_page_with_nikud_enhanced(self, page):
        """Анализ страницы с улучшенным объединением никуда"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И УЛУЧШЕННОЕ ОБЪЕДИНЕНИЕ НИКУДА")
        print("="*60)
        
        # Извлекаем слова
        raw_words = page.extract_words()
        
        # Улучшенное объединение никуда с буквами
        merged_words = merge_nikud_with_letters_enhanced(raw_words)
        
        print(f"  📊 До объединения: {len(raw_words)} слов")
        print(f"  🔗 После объединения: {len(merged_words)} слов")
        print(f"  📝 Объединено никуда: {len(raw_words) - len(merged_words)} символов")
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С УЛУЧШЕННЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с улучшенным объединением никуда
        processed_words = self._process_words_with_nikud_enhanced(merged_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С УЛУЧШЕННЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с улучшенным объединением никуда
        y_groups = self._form_lines_with_nikud_enhanced(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С УЛУЧШЕННЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 3: Статистика с улучшенным объединением никуда
        self._analyze_with_nikud_enhanced_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_nikud_enhanced_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_nikud_enhanced(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с улучшенным объединением никуда"""
        print(f"  🔤 Обработка {len(raw_words)} слов с улучшенным объединением никуда:")
    ...
    def _form_lines_with_nikud_enhanced(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с улучшенным объединением никуда"""
        print(f"  📋 Формирование строк с улучшенным объединением никуда ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_nikud_enhanced(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_nikud_enhanced(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с улучшенным объединением никуда"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_nikud_enhanced(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_nikud_enhanced(line)
    ...
    def _split_words_into_lines_nikud_enhanced(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с улучшенным объединением никуда"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_nikud_enhanced(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с улучшенным объединением никуда"""
        if not words:
            return ""
    ...
    def _analyze_with_nikud_enhanced_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с улучшенным объединением никуда"""
        print(f"  📊 Анализ {len(y_groups)} групп с улучшенным объединением никуда:")
    ...
    def _save_nikud_enhanced_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с улучшенным объединением никуда"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с улучшенным объединением никуда
        groups_file = os.path.join(debug_dir, f"{timestamp}_nikud_enhanced_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С УЛУЧШЕННЫМ ОБЪЕДИНЕНИЕМ НИКУДА\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с улучшенным объединением никуда сохранен:")
        print(f"     📄 Группы с улучшенным объединением никуда: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_nikud_enhanced.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = NikudEnhancedAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_nikud_enhanced(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с улучшенным объединением никуда завершен успешно")
    else:
