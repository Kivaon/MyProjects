...
@dataclass
class ProcessedWord:
    """Обработанное слово с учетом RTL/LTR"""
    text: str
    original_text: str
    x0: float
    y0: float
...
@dataclass
class ProcessedLine:
    """Обработанная строка с правильным RTL/LTR"""
...
class RTLEndFixedAnalyzer:
    """Анализатор с исправленным определением конца строки для RTL"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_rtl_end_fix(self, pdf_path: str) -> bool:
        """Основной метод анализа с исправленным концом строки для RTL"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_rtl_end_fix(page)
    ...
    def _analyze_page_with_rtl_end_fix(self, page):
        """Анализ страницы с исправленным концом строки для RTL"""
        print("\n" + "="*60)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА СЛОВ С УЧЕТОМ RTL/LTR")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с учетом RTL/LTR
        processed_words = self._process_words_with_rtl_end_fix(words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ПРАВИЛЬНЫМ КОНЦОМ")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с правильным концом
        y_groups = self._form_lines_with_rtl_end_fix(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ПРАВИЛЬНЫМ RTL/LTR")
        print("="*60)
        
        # ЭТАП 3: Статистика с правильным RTL/LTR
        self._analyze_with_rtl_end_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_rtl_end_fixed_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_rtl_end_fix(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с учетом RTL/LTR"""
        print(f"  🔤 Обработка {len(raw_words)} слов с учетом RTL/LTR:")
        ...
        for i, word in enumerate(raw_words):
            ...
            # НЕ инвертируем порядок букв для знаков препинания!
            ...
            if i < 15:
                ...
                print(f"    {i+1:2d}. {original_text:12s} → {processed_text:12s} {mark} ({word['x0']:6.1f},{word['top']:6.1f})")
            elif i == 15:
    ...
    def _form_lines_with_rtl_end_fix(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с правильным концом"""
        print(f"  📋 Формирование строк с правильным концом ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_rtl_end_fix(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_rtl_end_fix(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с правильным концом строки"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_rtl_end_fix(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_rtl_end_fix(line)
    ...
    def _split_words_into_lines_rtl_end_fix(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с правильным концом"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_rtl_end_fix(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с правильным концом строки"""
        if not words:
            return ""
        ...
        if direction == 'rtl':
            ...
            # Сортируем слова по X убыванию (справа налево)
    ...
    def _analyze_with_rtl_end_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с правильным RTL/LTR"""
        print(f"  📊 Анализ {len(y_groups)} групп с правильным RTL/LTR:")
    ...
    def _save_rtl_end_fixed_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с правильным RTL/LTR"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с правильным RTL/LTR
        groups_file = os.path.join(debug_dir, f"{timestamp}_rtl_end_fixed_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ПРАВИЛЬНЫМ RTL/LTR\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с правильным RTL/LTR сохранен:")
        print(f"     📄 Группы с правильным RTL/LTR: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_rtl_end_fix.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = RTLEndFixedAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_rtl_end_fix(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с правильным RTL/LTR завершен успешно")
    else:
