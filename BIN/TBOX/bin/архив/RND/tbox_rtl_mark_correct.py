...
# --- ПАСПОРТ ---
VERSION = "v3.16.rtl-mark-correct"
DATE    = "2026-05-01"
...
class RTLMarkCorrectAnalyzer:
    """Анализатор с исправленным RTL Mark для правильного порядка слов"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_rtl_mark_correct(self, pdf_path: str) -> bool:
        """Основной метод анализа с исправленным RTL Mark"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_rtl_mark_correct(page)
    ...
    def _analyze_page_with_rtl_mark_correct(self, page):
        """Анализ страницы с исправленным RTL Mark"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И ИСПРАВЛЕННОГО ПОРЯДКА СЛОВ")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С ИСПРАВЛЕННЫМ ПОРЯДКОМ СЛОВ")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с исправленным RTL Mark
        processed_words = self._process_words_with_rtl_mark_correct(words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ИСПРАВЛЕННЫМ ПОРЯДКОМ")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с исправленным порядком слов
        y_groups = self._form_lines_with_rtl_mark_correct(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ИСПРАВЛЕННЫМ ПОРЯДКОМ")
        print("="*60)
        
        # ЭТАП 3: Статистика с исправленным порядком слов
        self._analyze_with_rtl_mark_correct_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_rtl_mark_correct_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_rtl_mark_correct(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с исправленным RTL Mark"""
        print(f"  🔤 Обработка {len(raw_words)} слов с исправленным RTL Mark:")
    ...
    def _form_lines_with_rtl_mark_correct(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с исправленным порядком слов"""
        print(f"  📋 Формирование строк с исправленным порядком слов ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_rtl_mark_correct(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_rtl_mark_correct(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с исправленным порядком слов"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_rtl_mark_correct(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_rtl_mark_correct(line)
    ...
    def _split_words_into_lines_rtl_mark_correct(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с исправленным порядком слов"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_rtl_mark_correct(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с исправленным порядком слов - КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ"""
        if not words:
            return ""
        ...
        if direction == 'rtl':
            # Для RTL: НЕ сортируем по X, а используем оригинальный порядок
            # Слова уже в правильном порядке после обработки
            text_parts = []
            ...
            # Соединяем слова через пробелы - сохраняем оригинальный порядок
    ...
    def _analyze_with_rtl_mark_correct_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с исправленным порядком слов"""
        print(f"  📊 Анализ {len(y_groups)} групп с исправленным порядком слов:")
    ...
    def _save_rtl_mark_correct_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с исправленным порядком слов"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с исправленным порядком слов
        groups_file = os.path.join(debug_dir, f"{timestamp}_rtl_mark_correct_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ИСПРАВЛЕННЫМ ПОРЯДКОМ СЛОВ ДЛЯ RTL MARK\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с исправленным порядком слов сохранен:")
        print(f"     📄 Группы с исправленным порядком слов: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_rtl_mark_correct.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = RTLMarkCorrectAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_rtl_mark_correct(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с исправленным порядком слов завершен успешно")
    else:
