...
@dataclass
class ProcessedWord:
    """Обработанное слово с ячейковым объединением никуда"""
    text: str
    original_text: str
    x0: float
    y0: float
...
@dataclass
class ProcessedLine:
    """Обработанная строка с ячейковым объединением никуда"""
...
class NikudCellAnalyzer:
    """Анализатор с ячейковым объединением никуда"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_nikud_cell(self, pdf_path: str) -> bool:
        """Основной метод анализа с ячейковым объединением никуда"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_nikud_cell(page)
    ...
    def _analyze_page_with_nikud_cell(self, page):
        """Анализ страницы с ячейковым объединением никуда"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И ЯЧЕЕКОВОЕ ОБЪЕДИНЕНИЕ НИКУДА")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С ЯЧЕЕКОВЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с ячейковым объединением никуда
        processed_words = self._process_words_with_nikud_cell(merged_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ЯЧЕЕКОВЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с ячейковым объединением никуда
        y_groups = self._form_lines_with_nikud_cell(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ЯЧЕЕКОВЫМ ОБЪЕДИНЕНИЕМ НИКУДА")
        print("="*60)
        
        # ЭТАП 3: Статистика с ячейковым объединением никуда
        self._analyze_with_nikud_cell_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_nikud_cell_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_nikud_cell(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с ячейковым объединением никуда"""
        print(f"  🔤 Обработка {len(raw_words)} слов с ячейковым объединением никуда:")
    ...
    def _form_lines_with_nikud_cell(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с ячейковым объединением никуда"""
        print(f"  📋 Формирование строк с ячейковым объединением никуда ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_nikud_cell(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_nikud_cell(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с ячейковым объединением никуда"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_nikud_cell(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_nikud_cell(line)
    ...
    def _split_words_into_lines_nikud_cell(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с ячейковым объединением никуда"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_nikud_cell(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с ячейковым объединением никуда"""
        if not words:
            return ""
    ...
    def _analyze_with_nikud_cell_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с ячейковым объединением никуда"""
        print(f"  📊 Анализ {len(y_groups)} групп с ячейковым объединением никуда:")
    ...
    def _save_nikud_cell_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с ячейковым объединением никуда"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с ячейковым объединением никуда
        groups_file = os.path.join(debug_dir, f"{timestamp}_nikud_cell_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ЯЧЕЕКОВЫМ ОБЪЕДИНЕНИЕМ НИКУДА\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с ячейковым объединением никуда сохранен:")
        print(f"     📄 Группы с ячейковым объединением никуда: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_nikud_cell.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = NikudCellAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_nikud_cell(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с ячейковым объединением никуда завершен успешно")
    else:
