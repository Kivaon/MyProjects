...
def merge_nikud_with_letters(words: List[Dict]) -> List[Dict]:
    """Объединяет никуд с буквами на основе близости X координат"""
    if not words:
        return words
    
    direction = 'rtl'  # Предполагаем RTL для иврита
    ...
    while i < len(sorted_words):
        ...
        # Если это никуд, пытаемся присоединить к предыдущему слову
        if is_nikud(current_word['text']):
            # Ищем предыдущее слово с близкими Y координатами
            if merged_words:
                prev_word = merged_words[-1]
                y_tolerance = 3.0
                
                if abs(current_word['top'] - prev_word['top']) <= y_tolerance:
                    # Присоединяем никуд к предыдущему слову
                    prev_word['text'] += current_word['text']
                    # Обновляем X координаты
                    prev_word['x1'] = max(prev_word['x1'], current_word['x1'])
                    i += 1
                    continue
...
@dataclass
class ProcessedWord:
    """Обработанное слово с объединенным никудом"""
    text: str
    original_text: str
    x0: float
    y0: float
...
@dataclass
class ProcessedLine:
    """Обработанная строка с объединенным никудом"""
...
class NikudMergeAnalyzer:
    """Анализатор с объединением никуда с буквами"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_nikud_merge(self, pdf_path: str) -> bool:
        """Основной метод анализа с объединением никуда"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_nikud_merge(page)
    ...
    def _analyze_page_with_nikud_merge(self, page):
        """Анализ страницы с объединением никуда"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И ОБЪЕДИНЕНИЕ НИКУДА")
        print("="*60)
        
        # Извлекаем слова
        raw_words = page.extract_words()
        
        # Объединяем никуд с буквами
        merged_words = merge_nikud_with_letters(raw_words)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С ОБЪЕДИНЕННЫМ НИКУДОМ")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с объединенным никудом
        processed_words = self._process_words_with_nikud_merge(merged_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ОБЪЕДИНЕННЫМ НИКУДОМ")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с объединенным никудом
        y_groups = self._form_lines_with_nikud_merge(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ОБЪЕДИНЕННЫМ НИКУДОМ")
        print("="*60)
        
        # ЭТАП 3: Статистика с объединенным никудом
        self._analyze_with_nikud_merge_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_nikud_merge_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_nikud_merge(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с объединенным никудом"""
        print(f"  🔤 Обработка {len(raw_words)} слов с объединенным никудом:")
    ...
    def _form_lines_with_nikud_merge(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с объединенным никудом"""
        print(f"  📋 Формирование строк с объединенным никудом ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_nikud_merge(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_nikud_merge(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с объединенным никудом"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_nikud_merge(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_nikud_merge(line)
    ...
    def _split_words_into_lines_nikud_merge(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с объединенным никудом"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_nikud_merge(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с объединенным никудом"""
        if not words:
            return ""
    ...
    def _analyze_with_nikud_merge_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с объединенным никудом"""
        print(f"  📊 Анализ {len(y_groups)} групп с объединенным никудом:")
    ...
    def _save_nikud_merge_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с объединенным никудом"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с объединенным никудом
        groups_file = os.path.join(debug_dir, f"{timestamp}_nikud_merge_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ОБЪЕДИНЕННЫМ НИКУДОМ\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с объединенным никудом сохранен:")
        print(f"     📄 Группы с объединенным никудом: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_nikud_merge.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = NikudMergeAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_nikud_merge(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с объединенным никудом завершен успешно")
    else:
