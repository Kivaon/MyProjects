...
def analyze_group5_specific_format(pdf_path: str):
    """Анализирует символы в группе 5 (Y центр 246.7) и выводит в формате через запятую"""
    try:
        with pdfplumber.open(pdf_path) as pdf:
            if pdf.pages:
                ...
                # Находим группу 5 (Y центр около 246.7)
                group5_words = []
                target_y = 246.7
                y_tolerance = 5.0
                ...
                print("ГРУППА 5 (Y центр: 246.7) - ФОРМАТИРОВАННЫЙ ВЫВОД")
                print("="*70)
...
def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_debug_group5_specific.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    success = analyze_group5_specific_format(pdf_path)
