...
@dataclass
class ProcessedWord:
    """Обработанное слово с финальным RTL"""
    text: str
    original_text: str
    x0: float
    y0: float
...
@dataclass
class ProcessedLine:
    """Обработанная строка с финальным RTL"""
...
class RTLFinalAnalyzer:
    """Анализатор с финальным RTL"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_rtl_final(self, pdf_path: str) -> bool:
        """Основной метод анализа с финальным RTL"""
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_rtl_final(page)
    ...
    def _analyze_page_with_rtl_final(self, page):
        """Анализ страницы с финальным RTL"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И ФИНАЛЬНЫЙ RTL")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С ФИНАЛЬНЫМ RTL")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с финальным RTL
        processed_words = self._process_words_with_rtl_final(merged_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ФИНАЛЬНЫМ RTL")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с финальным RTL
        y_groups = self._form_lines_with_rtl_final(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ФИНАЛЬНЫМ RTL")
        print("="*60)
        
        # ЭТАП 3: Статистика с финальным RTL
        self._analyze_with_rtl_final_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_rtl_final_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_rtl_final(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с финальным RTL"""
        print(f"  🔤 Обработка {len(raw_words)} слов с финальным RTL:")
    ...
    def _form_lines_with_rtl_final(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с финальным RTL"""
        print(f"  📋 Формирование строк с финальным RTL ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_rtl_final(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_rtl_final(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с финальным RTL"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_rtl_final(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_rtl_final(line)
    ...
    def _split_words_into_lines_rtl_final(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с финальным RTL"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_rtl_final(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с финальным RTL - ВАРИАНТ 1: word.text + '\u200F'"""
        if not words:
            return ""
    ...
    def _analyze_with_rtl_final_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с финальным RTL"""
        print(f"  📊 Анализ {len(y_groups)} групп с финальным RTL:")
    ...
    def _save_rtl_final_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с финальным RTL"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с финальным RTL
        groups_file = os.path.join(debug_dir, f"{timestamp}_rtl_final_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ФИНАЛЬНЫМ RTL (ВАРИАНТ 1: word.text + '\\u200F')\n")
            f.write("="*60 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с финальным RTL сохранен:")
        print(f"     📄 Группы с финальным RTL: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_rtl_final.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = RTLFinalAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_rtl_final(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с финальным RTL завершен успешно")
    else:
