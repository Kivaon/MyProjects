...
class RTLMarkFixedAnalyzer:
    """Анализатор с исправленным RTL Mark для знаков препинания"""
    
    def __init__(self, debug_mode: bool = True):
        self.debug_mode = debug_mode
        self.language_info = None
    
    def analyze_page_with_rtl_mark_fixed(self, pdf_path: str) -> bool:
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if pdf.pages:
                    page = pdf.pages[0]
                    self._analyze_page_with_rtl_mark_fixed(page)
    ...
    def _analyze_page_with_rtl_mark_fixed(self, page):
        """Анализ страницы с исправленным RTL Mark"""
        print("\n" + "="*60)
        print("ШАГ 1: ОПРЕДЕЛЕНИЕ ЯЗЫКА И ИСПРАВЛЕННОГО RTL MARK")
        print("="*60)
        ...
        print("\n" + "="*60)
        print("ШАГ 2: ОБРАБОТКА С ИСПРАВЛЕННЫМ RTL MARK")
        print("="*60)
        
        # ЭТАП 1: Обрабатываем слова с исправленным RTL Mark
        processed_words = self._process_words_with_rtl_mark_fixed(words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 3: ФОРМИРОВАНИЕ СТРОК С ИСПРАВЛЕННЫМ RTL MARK")
        print("="*60)
        
        # ЭТАП 2: Формируем строки с исправленным RTL Mark
        y_groups = self._form_lines_with_rtl_mark_fixed(processed_words, font_size_estimate)
        
        print("\n" + "="*60)
        print("ШАГ 4: СТАТИСТИКА С ИСПРАВЛЕННЫМ RTL MARK")
        print("="*60)
        
        # ЭТАП 3: Статистика с исправленным RTL Mark
        self._analyze_with_rtl_mark_fixed_stats(y_groups, font_size_estimate)
        
        # Сохраняем результаты
        self._save_rtl_mark_fixed_analysis(y_groups, font_size_estimate)
    ...
    def _process_words_with_rtl_mark_fixed(self, raw_words: List[Dict], font_size: float) -> List[ProcessedWord]:
        """ЭТАП 1: Обрабатываем слова с исправленным RTL Mark"""
    ...
    def _form_lines_with_rtl_mark_fixed(self, words: List[ProcessedWord], font_size: float) -> List[YGroup]:
        """ЭТАП 2: Формируем строки с исправленным RTL Mark"""
        print(f"  📋 Формирование строк с исправленным RTL Mark ({self.language_info['direction'].upper()}):")
        ...
        for word in sorted_words:
            ...
            if current_y is None:
                ...
                if current_group_words:
                    group = self._create_y_group_with_rtl_mark_fixed(current_group_words, len(groups), font_size)
                    groups.append(group)
                current_group_words = [word]
                current_y = word_y
    ...
    def _create_y_group_with_rtl_mark_fixed(self, words: List[ProcessedWord], group_number: int, font_size: float) -> YGroup:
        """Создает группу Y с исправленным RTL Mark"""
        if not words:
            return None
        ...
        lines = self._split_words_into_lines_rtl_mark_fixed(words, adaptive_gap_threshold)
        ...
        for i, line in enumerate(lines):
            if line:
                ...
                text = self._assemble_text_with_rtl_mark_fixed(line)
    ...
    def _split_words_into_lines_rtl_mark_fixed(self, words: List[ProcessedWord], gap_threshold: float) -> List[List[ProcessedWord]]:
        """Разделяет слова на строки с исправленным RTL Mark"""
        if len(words) <= 1:
            return [words]
    ...
    def _assemble_text_with_rtl_mark_fixed(self, words: List[ProcessedWord]) -> str:
        """Собирает текст с исправленным RTL Mark"""
        if not words:
            return ""
    ...
    def _analyze_with_rtl_mark_fixed_stats(self, y_groups: List[YGroup], font_size: float):
        """ЭТАП 3: Статистика с исправленным RTL Mark"""
        print(f"  📊 Анализ {len(y_groups)} групп с исправленным RTL Mark:")
    ...
    def _save_rtl_mark_fixed_analysis(self, y_groups: List[YGroup], font_size: float):
        """Сохраняет результаты анализа с исправленным RTL Mark"""
        timestamp = datetime.now().strftime("%y%m%d_%H%M%S")
        debug_dir = "/Users/kivaonmac/Documents/AI_Lab/02_TXT/debug"
        os.makedirs(debug_dir, exist_ok=True)
        
        # Сохраняем группы с исправленным RTL Mark
        groups_file = os.path.join(debug_dir, f"{timestamp}_rtl_mark_fixed_groups.txt")
        with open(groups_file, 'w', encoding='utf-8') as f:
            f.write("АНАЛИЗ С ИСПРАВЛЕННЫМ RTL MARK ДЛЯ ЗНАКОВ ПРЕПИНАНИЯ\n")
            f.write("="*50 + "\n\n")
        ...
        print(f"\n  💾 Файл анализа с исправленным RTL Mark сохранен:")
        print(f"     📄 Группы с исправленным RTL Mark: {groups_file}")

def main():
    """Основная функция"""
    if len(sys.argv) < 2:
        print("Использование: python tbox_rtl_mark_fixed.py <pdf_file>")
        sys.exit(1)
    
    pdf_path = sys.argv[1]
    
    analyzer = RTLMarkFixedAnalyzer(debug_mode=True)
    success = analyzer.analyze_page_with_rtl_mark_fixed(pdf_path)
    
    if success:
        print(f"\n✅ Анализ с исправленным RTL Mark завершен успешно")
    else:
